...
SCHEMAS = {
    ...
    "implement": {
        "files": {"experiment.py": "Pythonコード全文"},
...
INSTRUCTIONS = {
    ...
    "implement": "承認済み計画に忠実なPython実装をJSONのfilesに返す。入口はexperiment.py。--seed INTEGER --output PATHを受け付け、指定PATHへUTF-8 JSON {\"seed\":INTEGER,\"baseline\":数値,\"treatment\":数値}を書き出す。主指標は計画のmetric。seedを両条件へ適用しデータ漏洩を防ぐ。計算から数値を得る。結果のハードコード禁止。必要なら同じディレクトリの補助.pyも返せる。ネットワーク・パッケージインストール・外部送信はプログラムへ埋め込まず、準備不足なら作業を失敗として報告する。入力データは読み取り専用。書き込みは指定outputと同じ試行ディレクトリのみ。",
...
def render(job, state, response_path, root=None):
    ...
    return ("# Research Loop Kit 作業票\n\n"
            ...
            f"適用する共通スキル:\n{skill}\n\n"
            f"研究設定と回答:\n```json\n{dump({'config': state['config'], 'answers': state['answers'], 'deepening': state.get('deepening'), 'history': state['history'], 'prior_research': state.get('prior_research')})}\n```\n\n"
            f"入力:\n```json\n{dump(job['payload'])}\n```\n\n"
