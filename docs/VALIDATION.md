...
65件のテスト成功（既存59件＋共通srcの実行・境界6件）。
複数実験・次サイクルで同じモジュールを実際にimportし、コードの再送なしで再利用できることを確認。
研究の最新srcを変更しても登録済み実験は固定版で実行すること、並列の競合変更を拒否すること、
パス逸脱・構文エラー・固定版の改変を拒否すること、研究分岐で共通コードが独立して引き継がれることを検査。
実験作成スキルの参照資料と形式も確認済み。

```console
python tools/verify_release.py --archive dist/research-loop-kit-0.6.0-source.zip --wheel dist/research_loop_kit-0.6.0-py3-none-any.whl
```

独立配布の動作実証では2サイクル20回の実行に加え、共通srcと短い入口の生成、実験間の使用版一致を検査する。
